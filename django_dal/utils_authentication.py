...
def authorize(request):
    if not request.user.is_authenticated:
        ...
        # if it had been done earlier (where jwt_authorized is checked) it would have been overwritten by authenticate/login
        # so I have to do it here at the end with the caution not to overwrite the keys of the orginal authentication
        if jwt_payload is not None:
            request.session.update(
                {k: v for k, v in jwt_payload.items() if
                 k not in ['_auth_user_id', '_auth_user_backend', '_auth_user_hash']}
            )
